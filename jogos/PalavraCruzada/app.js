...
function playChime(kind = "word"){
  ...
  o1.frequency.exponentialRampToValueAtTime(base * (kind === "level" ? 2.0 : 1.25), t0 + (kind === "level" ? 0.18 : 0.10));
  o2.frequency.exponentialRampToValueAtTime(base * (kind === "level" ? 2.4 : 1.6), t0 + (kind === "level" ? 0.22 : 0.12));

  g1.gain.setValueAtTime(0.0001, t0);
  g2.gain.setValueAtTime(0.0001, t0);

  g1.gain.exponentialRampToValueAtTime(kind === "level" ? 0.10 : 0.07, t0 + 0.02);
  g2.gain.exponentialRampToValueAtTime(kind === "level" ? 0.06 : 0.04, t0 + 0.02);

  g1.gain.exponentialRampToValueAtTime(0.0001, t0 + (kind === "level" ? 0.28 : 0.18));
  g2.gain.exponentialRampToValueAtTime(0.0001, t0 + (kind === "level" ? 0.34 : 0.22));

  o1.connect(g1); g1.connect(master);
  o2.connect(g2); g2.connect(master);

  master.gain.exponentialRampToValueAtTime(0.18, t0 + 0.02);
  master.gain.exponentialRampToValueAtTime(0.0001, t0 + (kind === "level" ? 0.36 : 0.24));

  o1.start(t0);
  o2.start(t0);
  o1.stop(t0 + (kind === "level" ? 0.40 : 0.26));
  o2.stop(t0 + (kind === "level" ? 0.40 : 0.26));
}
...
function updateClue(){
  const w = state.words.find(x => x.id === state.activeWordId);
  if (!w){
    setText(UI.clueText, "Toque em uma célula para selecionar uma palavra.");
    return;
  }
...
function showLoupe(x, y){
  ...
  UI.loupe.setAttribute("aria-hidden", "false");
  moveLoupe(x, y);
  updateLoupe();
}
...
function hideLoupe(){
  ...
  UI.loupe.setAttribute("aria-hidden", "true");
}
...
function moveLoupe(x, y){
  ...
  const py = clamp(y - 90, 10 + h/2, window.innerHeight - 10 - h/2);

  UI.loupe.style.left = `${px}px`;
  UI.loupe.style.top = `${py}px`;
}

function updateLoupe(){
  if (!state.loupeOn) return;
  const a = state.activeCell ? cellAt(state.activeCell.r, state.activeCell.c) : null;
  const w = state.words.find(x => x.id === state.activeWordId) || null;

  const num = a?.num ? String(a.num) : "";
  const letter = a?.value ? a.value : " ";
  const dir = w ? (w.dir === "across" ? "HORIZONTAL" : "VERTICAL") : (state.dir === "across" ? "HORIZONTAL" : "VERTICAL");

  setText(UI.loupeNum, num ? `#${num}` : " ");
  setText(UI.loupeLetter, letter);
  setText(UI.loupeDir, dir);
}
...
function rewardLevel(){
  ...
  if (centerEl){
    for (let i=0; i<10; i++){
      const id = setTimeout(() => spawnSpark(centerEl), i * 35);
      state.fxTimeouts.push(id);
    }
    const toEl = UI.coinLabel || document.querySelector(".hud-coins");
    for (let i=0; i<6; i++){
...
function clearFxTimeouts(){
  for (const id of state.fxTimeouts){
    clearTimeout(id);
  }
  state.fxTimeouts = [];
}
...
function onGridPointerMove(e){
  if (state.lastPointerType !== "touch") return;
  if (!state.loupeOn) return;
...
function bindUI(){
  ...
  on(UI.grid, "pointercancel", onGridPointerUp, { passive: true });
