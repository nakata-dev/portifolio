...
function seedPhrases(topicIds) {
  ...
  return [
    ...
    /*10 FRASES UTEIS EM CASA / APARTAMENTO*/
...
function nextPhrase() {
  ...
  if (nextIndex > q.length - 1) {
    toast("voce ja esta na ultima frase ✅");
    beep("tuk");
    return false;
  }
...
function prevPhrase() {
  ...
  const prevIndex = STATE.session.index - 1;
  if (prevIndex < 0) {
    toast("voce ja esta na primeira frase ✅");
    beep("tuk");
    return false;
  }

  STATE.session.index = prevIndex;
  STATE.session.phraseId = q[STATE.session.index];
  resetCountForPhrase(STATE.session.phraseId);
  saveState();
  return true;
...
function showCycleSheet(masteredNow) {
  ...
  sheet.innerHTML = `
    ...
    <div class="row">
      <button class="btn btn--ok btn--full" data-action="next">avançar ▶</button>
    </div>
  `;
}
...
function render105xEmptyState() {
  ...
  APP.innerHTML = `
    <div class="stack">
      <section class="card stack">
        <div class="studyTop">
          <div class="badge studyModeBadge">105x</div>
    ...
    <div class="stack">
      <section class="card stack viewRel" id="view105x">
        ...
        <div class="trainActionsBar">
          <button class="btn btn--ok btn--full trainActionMain" data-action="repeat">repeti e entendi</button>

          <div class="trainNavArrows" aria-label="navegação entre frases">
            <button class="navArrow" data-action="prev" aria-label="frase anterior" title="frase anterior">◀</button>
            <button class="navArrow" data-action="next" aria-label="próxima frase" title="próxima frase">▶</button>
          </div>
        </div>
