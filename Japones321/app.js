    ...
    <div class="stack landingLeanPage">
      <section class="card heroCard stack heroCard--landing heroCard--lean">
        ...
        <div class="heroActions">
          ...
          <button type="button" class="btn btn--ghost btn--full landingPremiumFixBtn landingCompareBtn" data-action="goCompare" onclick="window.NIHONGO321_GO_COMPARE && window.NIHONGO321_GO_COMPARE()">ver Premium</button>
        </div>
      </section>
      ...
      <section class="card landingSimpleCta">
        ...
        <div class="landingSimpleActions">
          ...
          <button type="button" class="btn btn--muted btn--full landingPremiumFixBtn landingCompareBtn" data-action="goCompare" onclick="window.NIHONGO321_GO_COMPARE && window.NIHONGO321_GO_COMPARE()">ver Premium</button>
        </div>
      </section>
    ...
    <div class="stack premiumThemesPage">
      ...
      <section class="card premiumThemesCta">
        ...
        <div class="premiumThemesActions">
          ...
          <button class="btn btn--muted btn--full" data-nav="#/compare">comparar planos</button>
...
function renderPlanCompare() {
  ...
  const freeItems = [
    ["Começar sem medo", "Treino 105x, Pack Essencial, frase do dia e favoritos."],
    ["Criar frases próprias", "Salve o japonês que aparece na sua vida real."],
    ["Compartilhar conteúdo", "Backup por WhatsApp/LINE sem apagar o app do colega."],
    ["Criar ritmo", "Poucos minutos por dia para manter contato com o idioma."]
  ];

  const premiumItems = [
    ["Situações que apertam", "Fábrica, chefe, hospital, prefeitura, contas, banco e moradia."],
    ["Trilhas com direção", "Caminhos para trabalho, vida adulta, conversa natural e gramática."],
    ["Sensei IA guiado", "Transforme sua dúvida em frases para treinar no 105x."],
    ["Mais clareza antes de falar", "Explicações, palavras importantes e contexto para não repetir no escuro."]
  ];

  const wins = [
    ["Conta veio diferente", "Você aprende a confirmar cobrança, plano, vencimento e desconto."],
    ["Chefe falou rápido", "Você treina como pedir repetição e confirmar a tarefa sem parecer rude."],
    ["Documento travou", "Você ganha frases para prefeitura, banco, hospital e moradia."],
    ["Dúvida específica", "O Sensei IA cria um mini-material para o seu caso real."]
  ];

  APP.innerHTML = `
    <div class="stack comparePage comparePageV2">
      <section class="card compareHero compareHeroV2">
        <div class="row row--between">
          <div class="badge">grátis x premium</div>
          <button class="btn" data-nav="#/landing">voltar</button>
        </div>

        <div class="compareHeroGrid">
          <div>
            <h1 class="compareTitle">O grátis te coloca em movimento. O Premium te prepara para a vida real.</h1>
            <p class="compareLead">
              A comparação é simples: o grátis cria hábito. O Premium reduz dependência quando a situação fica específica.
            </p>
          </div>

          <div class="compareHeroMini compareHeroMiniV2">
            <span>¥</span>
      ...
      <section class="comparePlanDuel">
        <div class="card comparePlan comparePlan--free comparePlanV2">
          <div class="comparePlanBadgeLine">
            <div class="badge">plano grátis</div>
            <div class="comparePlanTag">primeiro passo</div>
          </div>

          <div class="comparePlanHeadV2">
            <span class="comparePlanIcon">🧭</span>
            <div>
              <h2 class="h2 comparePlanTitle">Para começar hoje.</h2>
              <p class="p comparePlanText">
                Serve para sentir o método, treinar o essencial e criar constância sem pressão.
              </p>
            </div>
          </div>

          <div class="compareValuePhrase">
            “Eu consigo treinar um pouco mesmo cansado.”
          </div>

          <div class="compareList">
            ${freeItems.map(([title, text]) => `
          ...
          <button class="primaryAction" data-nav="#/home">começar grátis</button>
        </div>

        <div class="card comparePlan comparePlan--premium comparePlanV2">
          <div class="comparePlanBadgeLine">
            <div class="badge">plano Premium</div>
            <div class="comparePlanTag comparePlanTag--gold">mais segurança</div>
          </div>

          <div class="comparePlanHeadV2">
            <span class="comparePlanIcon comparePlanIcon--gold">🌅</span>
            <div>
              <h2 class="h2 comparePlanTitle">Para viver melhor no Japão.</h2>
              <p class="p comparePlanText">
                Serve quando você precisa falar melhor no trabalho, resolver documentos, contas, saúde e situações reais.
              </p>
            </div>
          </div>

          <div class="compareValuePhrase compareValuePhrase--gold">
            “Cada iene vira menos medo na hora de falar.”
          </div>

          <div class="compareList">
            ${premiumItems.map(([title, text]) => `
              <div class="compareItem compareItem--premium">
                <span>★</span>
                <div><b>${escapeHTML(title)}</b><small>${escapeHTML(text)}</small></div>
              </div>
            `).join("")}
          ...
          ${checkoutReady ? "" : `
            <div class="compareNotice">
              Checkout ainda não configurado. Antes da venda oficial, coloque o link real em SALES.checkoutUrl.
            </div>
          `}
        </div>
      </section>

      <section class="card compareMomentCard">
        <div class="row row--between">
          <div class="badge">onde o Premium brilha</div>
          <div class="badge">vida real</div>
        </div>

        <h2 class="h2 compareMomentTitle">O Premium vale quando o japonês deixa de ser “estudo” e vira necessidade.</h2>

        <div class="compareMomentGrid">
          ${wins.map(([title, text]) => `
            <div class="compareMomentItem">
      ...
      <section class="card compareDecisionCard">
        <div class="compareDecisionCopy">
          <div class="badge">decisão honesta</div>
          <h2 class="h2">Use grátis para começar. Assine quando quiser avançar sem se perder.</h2>
          <p class="p">
            O NIHONGO321 foi feito para brasileiros no Japão que têm pouco tempo, mas precisam de japonês útil amanhã.
          </p>
        </div>

        <div class="compareDecisionActions">
          <button class="primaryAction" data-action="checkout">
            ${escapeHTML(checkoutButtonLabel("primary"))}
          </button>
          <button class="btn btn--muted btn--full" data-nav="#/paths">ver trilhas</button>
          <button class="btn btn--ghost btn--full" data-nav="#/premium">detalhes do Premium</button>
        </div>
      </section>
    </div>
  `;
